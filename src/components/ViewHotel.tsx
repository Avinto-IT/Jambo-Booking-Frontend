...
import Hero from "./HotelHero/Hero";

const cleanFacilityName = (name: string) => name.replace(/[^a-zA-Z0-9]/g, "");

const ViewHotel: React.FC<{ hotel: Hotel }> = ({ hotel }) => {
  ...
  const incrementCount = (index: number) => {
    setCount((prevCounts) => {
      ...
      const maxCount = parseInt(hotel.rooms[index].numberOfRooms);
      if (newCounts[index] < maxCount) newCounts[index] += 1;
      return newCounts;
    });
  };
